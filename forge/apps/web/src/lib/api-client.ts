/**
 * Único ponto de contato do Forge com o organismo LUNA. Tudo aqui é HTTP
 * contra contratos já públicos e testados — Gateway e Cognitive Engine
 * (`/api/chat`). Nenhuma lógica de banco, memória ou provider vive aqui.
 */

const LUNA_API_BASE_URL = import.meta.env.VITE_LUNA_API_BASE_URL ?? "http://localhost:3001/api";
const FORGE_SERVER_URL = import.meta.env.VITE_FORGE_SERVER_URL ?? "http://localhost:4100";
...
export async function fetchLocalGitStatus(): Promise<LocalGitStatus> {
  const response = await fetch(`${FORGE_SERVER_URL}/api/git-status`);
  return parseJsonOrThrow<LocalGitStatus>(response);
}

export function terminalWebSocketUrl(): string {
  return FORGE_SERVER_URL.replace(/^http/, "ws") + "/terminal";
}
