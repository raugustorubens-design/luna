...
import { terminalWebSocketUrl } from "@/lib/api-client";
...
export function Terminal() {
  ...
  useEffect(() => {
    ...
    // dev, and fitAddon.fit() throws if called on an already-disposed
    // terminal caught mid-cycle. Harmless in production (no double-mount),
    // but worth guarding rather than leaving an uncaught error in the console.
    const safeFit = () => {
      if (disposed) return;
      try {
        fitAddon.fit();
